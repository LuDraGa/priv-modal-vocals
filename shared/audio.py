"""Shared audio processing utilities across all Modal voice services.

Includes PCM manipulation, WAV wrapping, normalization, stitching,
and audio validation for both reference and source audio.
"""
...
def validate_reference_audio(
    ...
    """Comprehensive validation for reference audio files used in cloning/conversion.
...
def validate_source_audio(
    ...
    """Validate source audio for voice conversion.

    Source audio has no duration constraints — it is the speech content to convert.
    Only size is enforced.
    ...
    if not audio_bytes:
        return AudioValidationResult(
            is_valid=False,
            error_message="Source audio file is empty",
        )

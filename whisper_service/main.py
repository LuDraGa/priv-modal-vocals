...
# ============================================================================
# Modal Class with @enter Hook
# ============================================================================

@app.cls(
    image=image,
    gpu="A10G",  # 24GB VRAM (holds both Whisper + Wav2Vec2 simultaneously)
    volumes={"/models": volume},  # Mount Volume at /models
    min_containers=0,  # Scale to zero when idle (cost-optimized)
    timeout=600,  # 10 min max per request (long audio files)
    enable_memory_snapshot=True,  # Faster cold starts (15s → 5-8s)
    scaledown_window=120,  # Keep container alive for 2min after request
)
class WhisperXService:
    """WhisperX STT service with word-level timestamps."""

    @modal.enter()
    def load_models(self):
        """Load WhisperX models on container startup.

        This hook is called once per container lifetime, before any requests.
        Models are loaded from the Modal Volume for fast startup.
        """
        ...
        logger.info("whisperx_service.loading_models")

        # Initialize and load WhisperX engine
        self.engine = WhisperXEngine(cache_dir="/models/hf_cache")
        self.engine.load_models()

        logger.info(
            "whisperx_service.models_loaded",
            gpu_available=self.engine.is_gpu_available(),
            alignment_available=self.engine.is_alignment_available(),
        )

    @modal.method()
    def transcribe(self, audio_path: str, language: str = None):
        """Transcribe audio (internal method for testing).
